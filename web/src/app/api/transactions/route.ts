...
async function proxyToBackend(req: NextRequest, endpoint: string) {
  try {
    const url = `${API_URL}/api/transactions/${endpoint}`;
...
/**
 * GET handler for transaction endpoints
 */
export async function GET(req: NextRequest) {
  ...
  if (path.startsWith('details/')) {
    const signature = path.replace('details/', '');
    return proxyToBackend(req, `details/${signature}`);
  } else if (path.startsWith('history/')) {
    const address = path.replace('history/', '');
    return proxyToBackend(req, `history/${address}`);
  } else {
...
/**
 * POST handler for transaction operations
 */
export async function POST(req: NextRequest) {
  ...
  if (path === 'submit') {
    return proxyToBackend(req, 'submit');
  } else {
    return NextResponse.json({ error: 'Not found' }, { status: 404 });
  }
} 